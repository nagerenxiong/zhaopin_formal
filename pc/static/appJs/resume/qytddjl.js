...
function paginBind(dataInfo){
    ...
    $(dataInfo).each(function(i) {
        ...
        if($.trim(array[14]) == "" || $.trim(array[14])=="&nbsp;"){
            array[14] = "";
        }
        else
            array[14] = "&nbsp;|&nbsp;"+array[14];
        if($.trim(array[15]) == "" || $.trim(array[15])=="&nbsp;")
            array[15] = "";
        else
            array[15] = "&nbsp;|&nbsp;"+array[15];
        ...
        content += '                <span onclick="doForwardResume([' + array[41] + '])" style="cursor: pointer;">转发</span>';
        var letter_text = letterDefaultContent(array[1], array[72], array[3], array[43], array[37]);
        content += '                <span data-toggle="modal" data-target="#sxmessage_Modal" onclick="getOperateDatass(this, \''+array[3]+'\', '+array[2]+',\'\', \'\', \'\',\''+encodeURIComponent(letter_text)+'\')" style="cursor: pointer;">私信</span>';
        content += '        </div></div>';
        content += '    </div>';
//        content +='<div style="clear:both; float:left; margin-left:19px;"><input name="choic_input" type="checkbox" value="'+array[0]+'"></div>';
        ...
        $(".o_info").each(function(index, el) {
            if($.trim($(this).text()).substr(0,1)=="|"){
            $(this).text($.trim($(this).text()).substr(1));
        }
        });
...
function dealResume(type){
    ...
    if(operateState == 'simple'){
        ...
        if(checkboxes.length == 0){
            window.alert("请选择要处理的简历！",4);
            return;
        }
        var ajInfoIds = [];
        for(var m = 0;m< checkboxes.length;m++){
            ajInfoIds.push(checkboxes.eq(m).val());
        }
        $.ajax({
            type: "POST",
            url: "/ajax_dealResume",
            data: {"ajInfoIds": ajInfoIds, "type": type, "content": content},
            dataType: "json",
            success: function (data) {
                if (data.msg != 'success') {
                    window.alert(data.msg,4);
                }else{
                    updateTabCount(type, checkboxes.length);
                    loadData = new pagin("paginInfo", "V_AJ_Resume_Deal", "", querySql, "", 6, "paginBind", "loadData", 4);
                    $('#reason').val('');
                    window.alert('处理完成！',4)
                }
