<!-- 转发简历入口方法 -->
var vIds=[];
function new_doForwardPosition(vId) {
                ...
                <ul class="f_l clearfix">\
                    <li class="index_li">\
                        ...
                        <div class="addr_box">\
                            ...
                            <div class="addr_text" style="width:50px;">\
                    ...
                    <div class="btn_list">\
                        <button onclick="forwardPosition()">发送</button>\
                        <a href="javascript:void(0);" id="closeBtn">取消</a>\
                    </div>\
                </ul>\
                <div class="f_r">\
                    <div class="lxr">\
                        <span class="lxr_1 active" title="联系人">\
                        </span><span class="lxr_2" title="分组"></span>\
                    </div>\
                    <div class="lxr_l">\
                        <div class="lxr_list">\
                            <p>最近联系人</p>\
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#forward_topic").val(data[0].pc_name+'招聘——' + data[0].position_name);
            $("#forward_content").val("该职位，我已查阅，觉得不错，特别推荐给你，请查收。");
        }
    });
...
//查询是否存在　返回hidden值
   ...
   //光标定位
    ...
    $(".lxr_r").on('click','.lxr_list_l',function(){
        ...
        $(".addr_box").children('.addr_li').remove();
        $(".addr_box .tip").hide();
         $(".addr_text").before('<div class="addr_li addr_zli">' + $(this).children('em').text() + ';<input type="hidden" value="' + $(this).children('em').attr('data-zd') + '"/></div>');
        $(".forward_type").val('2');
    })
...
function forwardPosition(){
    ...
    $.ajax({
        ...
        data: {"vids": vids, "emails": emails, "content": content, "forward_topic": forward_topic},
        dataType: "json",
        success: function(data){
            if(data.status == "1"){
                alert("发送成功!",1);
