...
// };
 SWFUpload.prototype.cleanUp = function (f) {  
        ...
        window.__flash__removeCallback = function (c, b) { try { if (c) { c[b] = null; } } catch (a) { } };  
    };  
		...
		init: function(options, swfUploadOptions) {
			...
			return this.each(function() {
				...
				var settings = $.extend({
					...
					uploader: '/fileupload/', // The path to the server-side upload script
					...
					height: 40, // The height of the browse button
				...
				if (flashInstalled) {
					...
					$wrapper.append("<a style='float:left;margin-top:10px;margin-left:10px' href=\"javascript:$('#" + settings.id + "').uploadify('upload', '*')\"class='start_upload'>上传</a>");
		...
		onSelect: function(file) {
			...
			if (settings.itemTemplate == false) {
				settings.itemTemplate = '<div id="${fileID}" class="uploadify-queue-item">\
					...
					<div class="oh" style="border-radius:5px;border:1px solid #ededed;padding: 16px 10px 16px 30px;background-color: #fbfbfb;margin-top: 10px;">\
					<div class="cancel">\
						<a href="javascript:$(\'#${instanceID}\').uploadify(\'cancel\', \'${fileID}\')">X</a>\
					</div><img src="../../static/images/scicon_03.png" style="margin-right:10px">\
					<span class="fileName" style="width:150px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;vertical-align: middle;display:inline-block">${fileName}</span>\
					</div><div><span class="data c_c8 mt10"></span></div></div>';
			}
			...
			if ($.inArray('onSelectError', settings.overrideEvents) < 0) {
				switch (errorCode) {
					...
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						...
						// this.queueData.errorMsg += '\n' +"图片大小不能大于"+ settings.fileSizeLimit + '.';
						break;
