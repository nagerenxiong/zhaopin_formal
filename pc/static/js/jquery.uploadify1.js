		...
		init: function(options, swfUploadOptions) {
			...
			return this.each(function() {
				...
				var settings = $.extend({
					...
					swf: '/static/js/uploadify.swf', // The path to the uploadify SWF file
					uploader: '/uploadFile/', // The path to the server-side upload script
					...
					height: 57, // The height of the browse button
					...
					width: 304, // The width of the browse button
				...
				if (flashInstalled) {
					...
					var $button = $('<div />', {
						...
						'class': 'uploader_btn ' + settings.buttonClass
					});
					...
					if (!settings.queueID) {
						var $queue = $('<div />', {
							...
							'class': 'uploader_jdt_box'
						});
						$(".uploader_jdt_box").append($queue);						
	...
	var handlers = {
		...
		onSelect: function(file) {
			...
			if (settings.itemTemplate == false) {
				// settings.itemTemplate="<a href=\"javascript:$('#" + settings.id + "').uploadify('upload', '*')\"class='start_upload' style='width:150px;height:40px;color:white;line-height: 40px;background-color:#d21d30;border:0;border-radius: 5px;margin: 0 auto;display:block;font-size:16px'>确  认</a>";
				settings.itemTemplate='<div class="uploadify-progress">\
				<div class="uploadify-progress-bar"></div></div>\
					<div id="${fileID}" class="uploadify-queue-item">\
					<div class="cancel">\
						<a style="margin-top:8px" href="javascript:$(\'#${instanceID}\').uploadify(\'cancel\', \'${fileID}\')">X</a>\
					</div><img src="../../static/images/scicon_03.png" style="margin-left:20px">\
					<a class="fileName" href="javascript:void(0)" style="text-decoration: underline;color:#666;margin-left:20px;font-size:14px">${fileName}</a><span class="data"></span>\
					</div>';
			}
			// if (settings.itemTemplate == false) {
			// 	// settings.itemTemplate="<a href=\"javascript:$('#" + settings.id + "').uploadify('upload', '*')\"class='start_upload' style='width:150px;height:40px;color:white;line-height: 40px;background-color:#d21d30;border:0;border-radius: 5px;margin: 0 auto;display:block;font-size:16px'>确  认</a>";
			// 	settings.itemTemplate='<div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div>';
			// 	settings.itemTemplate+='<div class="uploader_jdt">\
	  //  			<div style="width:170px;overflow:hidden;white-space:nowrap;">${fileName}</div><div class="jdt"><div class="m" style="width:60%"></div></div><div class="uploader_bfb">100%</div><div class="uploader_det">删除</div>\
	  //  			</div>';
			// }
			// Run the default event handler
		...
		onUploadSuccess: function(file, data, response) {
			...
			if (data['ret'] == '1') {
				//上传成功处理
				var i = data['save_name'].indexOf("static\\");
				var url_show = "..\\" + data['save_name'].substr(i);
//				$("#bigimg").attr('src', url_show);
//				$('#bigimg').Jcrop({
//					allowResize: true,
//					allowMove: true,
//					aspectRatio: 1,
...
//				        alert(coords.w+" "+ coords.h+" "+ coords.x+" "+coords.y+url_show)
//			        }
//				});
			} else {
				//上传失败处理
